...
> {
    ...
    async getPropertiesGroupedByStatus(request: TRequest, response: TResponse)
        : Promise<Record<string, Property[]>>
    {

        return {
            ['HTTP/1.1 200 OK']: [
                new Property('D:displayname', await this.resource.getDisplayName(request, response)),
                new Property('D:resourcetype', await this.resource.getResourceType(request, response) ),
                new Property('D:getcontentlength', (await this.resource.getContentLength(request, response)).toString()),
